...
def extract_and_chunk_pdf():
    ...
    print(f"PDF 파일 탐색 중: {pdf_path}")
    
    if not os.path.exists(pdf_path):
        print(f"에러: '{pdf_path}' 위치에 파일이 없습니다")
        print("data/ 폴더 안에 PDF 파일을 '2026_야구규칙.pdf'이름으로 넣어주세요.")
        return None

    # 2. PDF에서 텍스트 추출 (출처확인을 위해 페이지 번호 메타데이터로 추가)
    ...
    # 3. LangChain 텍스트 스플리터로 영리하게 쪼개기 (청킹) recursive, 즉 재귀적으로 영리하게 간격조절
    ...
    print(f"텍스트 청킹 완료 (총 {len(final_chunks)} 개의 문서 조각 생성됨)")
    
    # 4. [점검 사항] 청킹 예시 시각화 출력 (1번 조각과 2번 조각의 오버랩 확인용)
    if len(final_chunks) >= 2:
        ...
        print("\n 데이터 조각이 1개밖에 생성되지 않아 연속 청킹 예시를 출력할 수 없습니다.")
        print(f" 조각 1 내용:\n{final_chunks[0]['text']}\n")
