...
def upload_data_to_qdrant():
    ...
    if not chunks:
        print("❌ 에러: 청킹된 데이터가 없어 Qdrant 적재를 중단합니다.")
        return

    # --- [STEP 2] 로컬 인프라 및 모델 준비 ---
    print("\n🔄 2. 한국어 임베딩 모델 로드 중 (최초 실행 시 다운로드로 인해 시간이 조금 걸립니다)...")
    # 한국어를 지원하는 강력한 오픈소스 고성능 임베딩 모델
    ...
    # 로컬에 실행 중인 Qdrant 서버와 통신할 클라이언트 객체 생성
    ...
    try:
        if qdrant_client.collection_exists(collection_name=COLLECTION_NAME):
            print(f"🗑️ 기존에 존재하는 '{COLLECTION_NAME}' 컬렉션을 삭제합니다.")
    ...
    print(f"🔄 4. {len(chunks)}개의 텍스트 조각 임베딩 및 DB 업로드 시작...")
    ...
    print(f"🏆 대성공! 총 {len(points)}개의 야구 규칙 벡터가 Qdrant 로컬 DB(`qdrant_data/`)에 안전하게 저장되었습니다.")
